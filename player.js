class player {
  ...
  outOFView() {
    ...
    Xdist = Math.abs(Math.round(this.x / size) - Math.round(p.x / size));
    Ydist = Math.abs(Math.round(this.y / size) - Math.round(p.y / size));

    if (Xdist > FOVview || Ydist > FOVview) {
      return true;
    }
    return false;
  }
